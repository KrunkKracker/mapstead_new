...
class EditVisibilityTest {
    ...
    @Test
    fun `activeEditFeatureId is derived during point movement`() = runTest {
        ...
        assertEquals(featureId, viewModel.uiState.value.activeEditFeatureId)
    }

    @Test
    fun `activeEditFeatureId is derived during line editing`() = runTest {
        backgroundScope.launch(UnconfinedTestDispatcher(testScheduler)) { viewModel.uiState.collect {} }
    ...
    @Test
    fun `activeEditFeatureId is derived during polygon editing`() = runTest {
        backgroundScope.launch(UnconfinedTestDispatcher(testScheduler)) { viewModel.uiState.collect {} }
        
        val featureId = UUID.randomUUID()
        val feature = MapFeatureEntity(
            id = featureId, propertyId = testPropertyId, planId = testPlanId, layerId = testLayerId,
            geometryType = "POLYGON", geometryJson = "{\"type\":\"Polygon\",\"coordinates\":[[[0,0],[1,0],[1,1],[0,0]]]}",
            coordinateSpace = "G", styleJson = "{}", accuracySource = "M"
        )
        every { mapRepo.getFeaturesForLayer(testLayerId) } returns flowOf(listOf(feature))
        advanceUntilIdle()
    ...
    @Test
    fun `highlight is cleared after successful exit`() = runTest {
        backgroundScope.launch(UnconfinedTestDispatcher(testScheduler)) { viewModel.uiState.collect {} }
        ...
        viewModel.beginPersistedShapeEdit(featureId)
        advanceUntilIdle()
        assertNotNull(viewModel.uiState.value.activeEditFeatureId)
        
        // Modify to avoid early return in save
        ...
        assertNull("Active edit highlight must be cleared", viewModel.uiState.value.activeEditFeatureId)
    }
}
